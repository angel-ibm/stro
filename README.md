...
In real life, the white box you saw in previous pictures (labeled as "Event Generator") is a normal Raspberry Pi containing astronomical software and, additionally, a **Kafka Producer** which sends the pictures to a Kafka Broker hosted on the demo system. Actually, I have emulated the task of the white box with a python snippet. Otherwise, I would have to lend you my camera an my guiding system. Well, no... I won't do this.

### 3. Content transport, storage and similarity search

Finally, the idea is that we get the pictures generated by the telescope as **Kafka events**. This is possible because the pictures itself are not so big and Kafka kan handle them. We need to store for some time the events until we are sure that can be disposed.
